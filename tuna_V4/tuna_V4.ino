...
#define FIREBASE_API_KEY "AIzaSyCsO1JUdMeoIe-BJgxg_vLVIgPe3kAyPsQ"
...
void loop() {
  ...
  if (!isnan(v[3]) && v[3] < 80.0) {
    combined += "Depan Bawah ";
    vibrate = true;
  }
  // Slope conditions
  if (!isnan(v[4]) && v[4] > 150.0) {
    combined += "Turunan Dalam ";
    vibrate = true;
  ...
  switch (currentState) {
    ...
    case WIFI_CONNECTED:
      {
        ...
        pCaneStatusCharacteristic->notify();
        // ssl_client.setInsecure(); // Not needed for old library
        config.host = FIREBASE_HOST;
        config.api_key = FIREBASE_API_KEY;
        auth.token.id_token = id_token_str;  // Use ID token for auth
    ...
    case FIREBASE_INITIALIZING:
      ...
      break;
    case FIREBASE_READY:
      // app.loop(); // Not needed for old library
